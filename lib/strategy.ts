...
export function calculateBOLL(klineData, period = 20, multiplier = 2) {
  ...
  for (let i = 0; i < klineData.length; i++) {
    ...
    let squareSum = 0
    for (let j = 0; j < period; j++) {
      squareSum += Math.pow(klineData[i - j].close - ma, 2)
    }
...
export function runBollStrategy(bollData) {
  ...
  for (let i = 1; i < bollData.length; i++) {
    ...
    if (position === 0 && prev.close <= prev.lower && curr.close > curr.lower) {
      ...
      position = 1
    } else if (position === 1 && prev.close >= prev.upper && curr.close < curr.upper) {
      // 卖出信号
      signals.push({
        date: curr.date,
        type: "SELL",
...
export function backtest(klineData, signals, initialCapital = 100000) {
  ...
  for (const signal of signals) {
    ...
    if (signal.type === "BUY") {
      ...
      trades.push({
        ...
        costPercentage: ((actualCosts.totalCost / (shares * price)) * 100).toFixed(4) + "%",
        capital,
      ...
      trades.push({
        ...
        costPercentage: ((costs.totalCost / (shares * price)) * 100).toFixed(4) + "%",
        capital,
        total: capital,
        index,
      })
  ...
  // 计算总交易成本
  const totalCosts = totalCommission + totalTransfer + totalStamp + totalSlippage

  // 计算交易成本对收益的影响
  const returnsWithoutCosts = ((finalAsset + totalCosts - initialCapital) / initialCapital) * 100
